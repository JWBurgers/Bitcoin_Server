...
For convenience, you should definitely set up the binaries, startup scripts, and associated data from your core Bitcoin services in a common base directory on a single drive. You can do this on a dedicated Bitcoin drive on your server, or on a more comprehensive drive that also includes your operating system and other applications and data (e.g., Tor, Fail2Ban, SSH). 

Whatever choice you make, the key variable you will need to consider in your hardware choices is storage space. We will set up Bitcoin Core with an entire history of the Block Chain, which is currently just over 500 GB (November 2023). In addition, we will set up Bitcoin Core with a transaction index. Having the entire Block Chain stored on your system benefits your security. In addition, both of the aforementioned configurations are really convenient, particularly as they are sometimes a prerequisite for other services that you might eventually run on a Bitcoin server. A complete Block Chain with transaction index is, for instance, required for a fully functioning local block explorer.  
